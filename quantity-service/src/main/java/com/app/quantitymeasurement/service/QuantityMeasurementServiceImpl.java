...
public class QuantityMeasurementServiceImpl implements IQuantityMeasurementService {
    ...
    public List<QuantityMeasurementDTO> getHistoryByOperation(OperationType operationType) {
        return repository.findByOperation(operationType).stream().map(this::toDto).toList();
    }

    @Override
    public List<QuantityMeasurementDTO> getHistoryByMeasurementType(String measurementType) {
        return repository.findByThisMeasurementType(MeasurementTypeResolver.normalizeMeasurementType(measurementType))
                .stream()
                .map(this::toDto)
                .toList();
    }
    ...
    public List<QuantityMeasurementDTO> getErroredHistory() {
        return repository.findByErrorTrue().stream().map(this::toDto).toList();
    }
    ...
    private QuantityMeasurementDTO saveSuccess(QuantityMeasurementDTO response) {
        QuantityMeasurementEntity saved = repository.save(toEntity(response));
        return toDto(saved);
    }
    ...
    private QuantityMeasurementDTO handleFailure(
        ...
        repository.save(toEntity(QuantityMeasurementDTO.failure(
                operationType,
                q1,
                q2,
    ...
    private QuantityMeasurementEntity toEntity(QuantityMeasurementDTO dto) {
        QuantityMeasurementEntity entity = new QuantityMeasurementEntity();
        entity.setId(dto.getId());
        entity.setOperation(dto.getOperationType());
        entity.setThisValue(dto.getThisValue());
        entity.setThisUnit(dto.getThisUnit());
        entity.setThisMeasurementType(dto.getThisMeasurementType());
        entity.setThatValue(dto.getThatValue());
        entity.setThatUnit(dto.getThatUnit());
        entity.setThatMeasurementType(dto.getThatMeasurementType());
