...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleQuantityException(RuntimeException exception, HttpServletRequest request) {
    ...
    private ResponseEntity<ApiErrorResponse> buildResponse(HttpStatus status, String error, String message, String path) {
        return ResponseEntity.status(status)
                .body(new ApiErrorResponse(LocalDateTime.now(), status.value(), error, message, path));
    }
}
